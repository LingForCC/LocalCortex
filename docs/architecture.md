...
This document is the authoritative reference for the architectural decisions made during design. The companion [Rules](./features/rules/README.md) doc specifies the user-facing config format.
...
The only structural selector is **`mcpServers`** — which MCP servers to attach to the run. This exists not to duplicate the rule's intent, but to **curate the agent's toolset**: function-calling models degrade as the tool list grows, so a user with six configured servers (two GitHub accounts, GitLab, Jira, Todoist, OmniFocus) must narrow the toolset per rule or the agent starts calling the wrong server or hallucinating tools. `mcpServers` is required and also bounds credential blast radius — a rule can only touch the systems its servers connect to. See [MCP sources → why mcpServers is the only structural selector](./features/mcp-sources/README.md#why-mcpservers-is-the-only-structural-selector).

**Stopping a rule** when its goal is met is handled in the **prompt contract**: the app appends a status contract requiring the agent to emit `{"status":"active|done|error"}` each run, and the app disables the rule on `done`/`error`. See [Rules → prompt contract](./features/rules/README.md#prompt-contract-app-assembled) and [§6.6](./architecture.md#66-stop-conditions--agent-signaled-completion--structural-backstop).
...
A rule is one or the other, not both. This keeps the scheduler and the event ingress as separate, simple code paths. See [Triggers](./features/triggers/README.md).
...
│   ├── features/rules/README.md   ← rule config format (fields, types, validation)
│   └── ... (other feature docs under features/)
├── src/
