...
- **Codex** — the workdir is where the SDK launches; `.codex/config.toml` in that same dir supplies MCP config. The two concerns converge: one per-run staged directory holds both.

Per-run staging under `~/.localcortex/runs/<rule-id>/<timestamp>/` for Codex, with a generated `.codex/config.toml` declaring the rule's MCP servers and `approval_policy = "never"`.
