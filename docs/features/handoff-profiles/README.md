...
A **handoff profile** binds together the three choices that specialize
LocalCortex around its core use case — creating review subtasks automatically
when a coding agent finishes a task:
...
> Previously this feature was a **singleton**: exactly one profile, stored as
> four scalar fields on `app_settings` plus one fixed-id rule (`handoff-auto`),
> set up via a first-run onboarding wizard. Migration 006 moves to the
> multi-profile model: profiles live in their own table, the wizard is gone,
> and any existing singleton setup is migrated into one profile row (see
> §Migration below).
...
A common multi-profile layout:

| Profile | Agent | Task manager | Backend |
| --- | --- | --- | --- |
...
## Managing handoff profiles

Handoff profiles are managed in the **Handoff profiles** tab (a primary tab in
the shell). The UI is a standard list-CRUD table (mirrors Rules / Sources):

- Each row shows the profile's label, agent, task manager, backend, an
  enable/disable switch, and Edit / Delete actions.
- **New handoff profile** / **Edit** opens an inline editor with a label field
  and the three pickers (agent, task manager, backend), reusing the same
  catalog picker primitives (including "+ Add custom…") as before.
- There is **no first-run wizard** and no onboarding gate — the app opens to
  the normal shell, and an empty profile list simply prompts you to create one
  from the Handoff profiles tab (or via Home → "Manage handoff profiles").

The Home tab summarizes the configured profiles and recent handoffs.

---

## What creating a handoff profile does

When you save a new profile, `handoff-profiles:create` atomically:
...
2. **Builds** a fresh rule (a normal `Rule`, per-profile UUID id) with:
...
### Profile ↔ rule ownership invariant

The profile **owns** its rule but **preserves your edits**:

- **Update profile** (agent / task manager / backend / label changed) → only
  the profile-owned rule fields are overwritten (`trigger.eventType`,
  `mcpServers`, `backend`, `name`). Your edits to the prompt, model, reasoning
  effort, sandbox, workdir, maxRuns, expiresAt, and notes are preserved.
- **Toggle enabled** → the flag is mirrored onto both the profile and its rule
  (the scheduler fires rules, not profiles, so the rule's `enabled` is what
  counts).
- **Delete profile** → the rule is deleted first, then the profile.

---

## Storage: the `handoff_profiles` table

Profiles live in a dedicated table ([migration 006](../../../src/main/db/migrations/006_handoff_profiles.sql)),
not in the `app_settings` JSON blob:

```sql
CREATE TABLE handoff_profiles (
  id              TEXT PRIMARY KEY,
  label           TEXT NOT NULL,
...
FK enforcement is ON (`DB_PRAGMAS`), so a catalog row referenced by a profile
can't be deleted while the profile exists. The repository + IPC layers are the
only place that coordinates the profile and its rule (see the ownership
invariant above).
...
Migration 006 copies any pre-existing singleton setup into one profile row,
reusing the existing `handoff-auto` rule. It reads the four `handoff*` keys
from the `app_settings` JSON blob via `json_extract` and inserts a profile with
`id = 'profile-handoff-auto'` **only if**:
...
the user simply starts fresh in the Handoff profiles tab. No data is lost in
either case.
...
CRUD-able in-app (the profile editor's "+ Add custom…" buttons). This is the
...
1. **Sources** tab → "Add server" → add the MCP server (form or paste JSON from
   the server's README).
2. Handoff profiles tab → edit/create a profile → task manager picker → "Add
   custom…" → fill in the label, description, select the MCP server, and
...
1. Handoff profiles tab → edit/create a profile → agent picker → "Add custom…"
   → fill in the label, event types (`myagent.session-complete`,
   `myagent.prompt-submit`), source, and install instructions.
...
The agent is immediately selectable. One agent can be used by multiple
profiles.
...
- **Home** (default tab) — profile summary (each agent → task manager → backend
  + status) + recent handoffs + "Manage handoff profiles".
- **Handoff profiles** — the profile list-CRUD table.
- **Handoffs** — the existing handoff list panel.
- **Run history** — the existing run observability view.
- **Rules** *(Advanced)* — the existing rule editor. Each profile's
  auto-created rule is visible and editable here.
...
- [Handoffs](../handoffs/README.md) — the review-subtask pipeline profiles power.
- [Rules](../rules/README.md) — each profile's auto-created rule is a normal rule.
