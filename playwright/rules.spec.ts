...
test.describe('Rules CRUD (R-E1..R-E5)', () => {
  ...
  test('R-E4: delete a rule removes the row', async ({ app }) => {
    ...
    // Scope Delete to the specific row (the handoff profile's auto-created rule also has a Delete).
    await window
  ...
  test('R-E5: empty rule text shows inline validation and saves nothing', async ({ app }) => {
    ...
    // Cancel back to the list. The handoff profile's auto-created rule is still there,
