 ...
 * fresh default ~/.localcortex/mcp-servers.json on launch with the four v1
 * servers (github, gitlab, todoist, omnifocus). Three of them (github, gitlab,
 * todoist) still carry the `<your-token-here>` placeholder; omnifocus has an
 * empty env, so it is NOT flagged.
 */
...
test.describe('MCP sources', () => {
  test('M-E1: default servers are provisioned and placeholders flagged', async ({ app }) => {
    ...
    // omnifocus (empty env) renders bare. Match the exact chip text.
  ...
  test('M-E4: deleting the config then relaunching recreates it with 0600 perms', async ({
    ...
    // The recreated file still parses and lists the four defaults.
    const cfg = JSON.parse(readFileSync(path, 'utf8'));
